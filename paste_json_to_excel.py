...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert Rakuten official Test Form JSON into CSV and Excel"
    )
    parser.add_argument("--input", default="downloaded_rakuten.json", help="Input JSON file")
...
def main() -> None:
    ...
    csv_path, excel_path = save_rankings(items, Path(args.output_dir), args.genre_id)
    print(f"Imported {len(items)} rows from {input_path}")
...
def extract_items(payload: dict) -> list[dict]:
    ...
    raise ValueError("Cannot find Items list in downloaded_rakuten.json")
